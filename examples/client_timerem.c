...
static void cbfunc(pmix_status_t status,
                   ...
                   void *release_cbdata)
{
    fprintf(stderr, "Callback: cbdata=%x info=%x\n", cbdata, info);
    ...
    /* indicate that our data is now ready */
    *waiting = false;
}
...
*/
    ...
    fprintf(stderr, "Hello Client %s:%d universe size %d\n", myproc.nspace, myproc.rank, nprocs);
    if (myproc.rank == 0) {
        ...
        fprintf(stderr, "Before Client %s:%d universe size %d\n", myproc.nspace, myproc.rank, nprocs);
        waiting = true;
        ...
        fprintf(stderr, "Client ns %s rank %d: Seconds remaining: %d\n", myproc.nspace, myproc.rank, seconds_remaining);
    }
